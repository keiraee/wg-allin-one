...
- **全隧道（可选）**：客户端 `AllowedIPs = 0.0.0.0/0, ::/0`；服务端启用转发 + NAT。
  强制带历史踩坑对策（必须全部满足才允许开启）：
  1. 服务端 `Table = off` + 策略路由，防止 `0.0.0.0/0` 把 SSH 吸进隧道断连；
  2. `iif wg0` 显式转发规则；
  3. NAT 伪装（masquerade）+ mtu_fix；
  4. 开启前脚本自检 SSH 会话来源，装完打印「新 SSH 会话验证通过才算成功」检查点。
...
| 坑 | 对策（已入设计） |
|---|---|
| VPS wg-quick 配 `0.0.0.0/0` 吸走 SSH 断连 | 全隧道强制 Table=off + 策略路由 + 自检 |
